...
static char* cli_show_devices(struct ast_cli_entry* e, int cmd, struct ast_cli_args* a)
{
	...
	static const char FORMAT1[] =
		"%-12.12s %-5.5s %-10.10s %-4.4s %-4.4s %-7.7s %-14.14s %-17.17s %-16.16s %-16.16s %-14.14s\n";
	static const char FORMAT2[] =
		"%-12.12s %-5d %-10.10s %-4d %-4d %-7s %-14.14s %-17.17s %-16.16s %-16.16s %-14.14s\n";
	...
	ast_cli(a->fd, FORMAT1, "ID", "Group", "State", "RSSI", "Mode", "Provider Name", "Model", "Firmware", "IMEI",
			"IMSI", "Number");

	AST_RWLIST_RDLOCK(&gpublic->devices);
	AST_RWLIST_TRAVERSE(&gpublic->devices, pvt, entry) {
		ast_mutex_lock(&pvt->lock);
		ast_cli(a->fd, FORMAT2, PVT_ID(pvt), CONF_SHARED(pvt, group), pvt_str_state(pvt), pvt->rssi, pvt->act,
...
static char* cli_show_device_settings(struct ast_cli_entry* e, int cmd, struct ast_cli_args* a)
{
	switch (cmd) {
		case CLI_GENERATE:
			if (a->pos == 4) { return complete_device(a->word, a->n); }
			return NULL;
	}
	...
	if (pvt) {
		...
		ast_cli(a->fd, "  Default CallingPres     : %s\n",
				S_COR(CONF_SHARED(pvt, callingpres) < 0, "<Not set>",
					  ast_describe_caller_presentation(CONF_SHARED(pvt, callingpres))));
...
CLI_ALIASES(cli_show_device_settings, "show device settings", "show device settings <device>",
...
static char* cli_show_device_state(struct ast_cli_entry* e, int cmd, struct ast_cli_args* a)
{
	...
	if (pvt) {
		...
		if (CONF_SHARED(pvt, query_time)) { ast_cli(a->fd, "  Module time             : %s\n", pvt->module_time); }
	...
	if (pvt) {
		...
		*/
		ast_cli(a->fd, "  ASR for incoming calls      : %d\n",
				getASR(PVT_STAT(pvt, in_calls) + PVT_STAT(pvt, cw_calls),
					   PVT_STAT(pvt, calls_answered[CALL_DIR_INCOMING])));
...
CLI_ALIASES(cli_show_device_statistics, "show device statistics", "show device statistics <device>",
...
static char* cli_show_version(struct ast_cli_entry* e, int cmd, struct ast_cli_args* a)
{
	...
	ast_cli(a->fd, "\n%s: %s, Version %s, Revision %s\nProject Home: %s\nBug Reporting: %s\n\n", AST_MODULE,
			MODULE_DESCRIPTION, MODULE_VERSION, PACKAGE_REVISION, MODULE_URL, MODULE_BUGREPORT);
...
static char* cli_cmd(struct ast_cli_entry* e, int cmd, struct ast_cli_args* a)
{
	switch (cmd) {
		case CLI_GENERATE:
			if (a->pos == 2) { return complete_device(a->word, a->n); }
			return NULL;
	}
	...
	ast_cli(a->fd, "[%s] '%s' %s\n", a->argv[2], a->argv[3],
			res < 0 ? error2str(chan_quectel_err) : "AT command queued");

	return CLI_SUCCESS;
}

CLI_ALIASES(cli_cmd, "cmd", "cmd <device> <command>",
			"Send <command> to the rfcomm port on the device with the specified <device>")
...
static char* cli_sms_send(struct ast_cli_entry* e, int cmd, struct ast_cli_args* a)
{
	...
	switch (cmd) {
		case CLI_GENERATE:
			if (a->pos == 3) { return complete_device(a->word, a->n); }
			return NULL;
	}
	...
	int res = send_sms(a->argv[3], a->argv[4], ast_str_buffer(buf), DEF_VALIDITY, DEF_REPORT, DEF_PAYLOAD,
					   STRLEN(DEF_PAYLOAD));
	ast_free(buf);
...
CLI_ALIASES(cli_sms_send, "sms send", "sms send <device> <number> <message>",
...
static char* cli_sms_list_received_unread(struct ast_cli_entry* e, int cmd, struct ast_cli_args* a)
{
	switch (cmd) {
		case CLI_GENERATE:
			if (a->pos == 5) { return complete_device(a->word, a->n); }
			return NULL;
	}
...
CLI_ALIASES(cli_sms_list_received_unread, "sms list received unread", "sms list received unread <device>",
			"List unread received mesages from <device>")
...
CLI_ALIASES(cli_sms_list_received_read, "sms list received read", "sms list received read <device>",
			"List read received mesages from <device>")
...
CLI_ALIASES(cli_sms_delete_received_read, "sms delete received read", "sms delete received read <device>",
			"Delete read mesages from <device>")
...
CLI_ALIASES(cli_sms_direct_off, "sms direct off", "sms direct off <device>",
			"Receive messages from <device> indirectly")
...
CLI_ALIASES(cli_sms_direct_auto, "sms direct auto", "sms direct auto <device>",
			"Receive messages from <device> in a way specified in configuration")
...
static char* cli_ccwa_set(struct ast_cli_entry* e, int cmd, struct ast_cli_args* a)
{
	...
	switch (cmd) {
		case CLI_GENERATE:
			if (a->pos == 2) { return ast_cli_complete(a->word, (ast_cli_complete2_t)choices, a->n); }
			if (a->pos == 3) { return complete_device(a->word, a->n); }
			return NULL;
	}
	...
	ast_cli(a->fd, "[%s] %s\n", a->argv[3],
			res < 0 ? error2str(chan_quectel_err) : "Call-Waiting commands queued for execute");

	return CLI_SUCCESS;
}

CLI_ALIASES(cli_ccwa_set, "callwaiting", "callwaiting disable|enable <device>",
...
static char* cli_restart_event(struct ast_cli_entry* e, int cmd, struct ast_cli_args* a, dev_state_t event)
{
	...
	if (a->argc != 4 && a->argc != 5) { return CLI_SHOWUSAGE; }

	for (i = 0; a_choices[i]; i++) {
		if (strcasecmp(a->argv[2], a_choices[i]) == 0) {
			if (i == RESTATE_TIME_CONVENIENT) {
				if (a->argc == 5 && strcasecmp(a->argv[3], a_choices2[0]) == 0) { device = a->argv[4]; }
			} else if (a->argc == 4) {
				device = a->argv[3];
			}
...
static char* cli_start(struct ast_cli_entry* e, int cmd, struct ast_cli_args* a)
{
	...
	ast_cli(a->fd, "[%s] %s\n", a->argv[2],
			res < 0 ? error2str(chan_quectel_err) : dev_state2str_msg(DEV_STATE_STARTED));
...
static char* cli_reload(struct ast_cli_entry* e, int cmd, struct ast_cli_args* a)
{
	...
	for (i = 0; a_choices[i]; i++) {
		if (strcasecmp(a->argv[2], a_choices[i]) == 0) {
			if (i == RESTATE_TIME_CONVENIENT) {
				if (a->argc == 4 && strcasecmp(a->argv[3], a_choices2[0]) == 0) { ok = 1; }
			} else if (a->argc == 3) {
				ok = 1;
			}
...
static char* cli_discovery(struct ast_cli_entry* e, int cmd, struct ast_cli_args* a)
{
	...
	for (res = pdiscovery_list_begin(&item); res; res = pdiscovery_list_next(&item)) {
		AST_RWLIST_TRAVERSE(&gpublic->devices, pvt, entry) {
			if (strcmp(PVT_STATE(pvt, data_tty), res->ports.ports[INTERFACE_TYPE_DATA]) == 0) { break; }
		}
		if (pvt) {
			/*
			...
			*/
			...
			ast_cli(a->fd, "[dc_%s_%s](defaults)\n", imei + imeilen - MIN(imeilen, 4),
					imsi + imsilen - MIN(imsilen, 4));
...
static char* cli_audio_loop(struct ast_cli_entry* e, int cmd, struct ast_cli_args* a)
{
	...
	switch (cmd) {
		case CLI_GENERATE:
			if (a->pos == 4) { return ast_cli_complete(a->word, (ast_cli_complete2_t)choices, a->n); }
			if (a->pos == 3) { return complete_device(a->word, a->n); }
			return NULL;
	}
...
CLI_ALIASES(cli_audio_mode, "audio mode", "audio mode <device> [off|general|handset|headset|speaker|bluetooth]",
...
static char* cli_audio_gain_tx(struct ast_cli_entry* e, int cmd, struct ast_cli_args* a)
{
	switch (cmd) {
		case CLI_GENERATE:
			if (a->pos == 5) { return ast_cli_complete(a->word, (ast_cli_complete2_t)audio_gain_choices, a->n); }
			if (a->pos == 4) { return complete_device(a->word, a->n); }
			return NULL;
	}
	...
	// write
	int gain;
	if (str2gain(a->argv[5], &gain)) { return CLI_SHOWUSAGE; }
...
CLI_ALIASES(cli_audio_gain_tx, "audio gain tx", "audio gain tx <device> [level]",
