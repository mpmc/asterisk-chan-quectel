...
static void* saturated_sum(void* s1, const void* s2, size_t n)
{
	...
	/* FIXME: odd bytes */
	for (n /= 2; n; n--, s11++, s22++) { ast_slinear_saturated_add(s11, s22); }
...
size_t mixb_write(struct mixbuffer* mb, struct mixstream* stream, const char* data, size_t len)
{
	...
	if (max_mix < len) { len = max_mix; }

	if (len > 0) {
		max_mix = mb->rb.used - stream->used;
		if (len > max_mix) {
			/* optitional Mix followed by copy */
			if (max_mix) { mixb_mix_write(mb, stream, data, max_mix); }
...
size_t mixb_read_upd(struct mixbuffer* mb, size_t len)
{
	...
	AST_LIST_TRAVERSE(&mb->streams, stream, entry) {
		...
		if (stream->write >= mb->rb.size) { stream->write -= mb->rb.size; }
	}
