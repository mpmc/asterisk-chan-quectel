...
int pdu_build_mult(pdu_part_t* pdus, const char* sca, const char* dst, const uint16_t* msg, size_t msg_len, unsigned valid_minutes, int srr, uint8_t csmsref)
{
    ...
    if (gsm7_len >= 0) {
        ...
        while (off < msg_len) {
            ...
            udh.order      = i + 1;
            udh.parts      = cnt;
            ssize_t curlen = pdu_build(pdus[i].buffer, PDU_LENGTH, &pdus[i].tpdu_length, sca, dst, PDU_DCS_ALPHABET_7BIT, msg_gsm7 + off, n, septets,
                                       valid_minutes, srr, &udh);
            if (curlen < 0) {
        ...
        while (off < msg_len) {
            ...
            udh.parts = cnt;
            ssize_t curlen =
                pdu_build(pdus[i].buffer, PDU_LENGTH, &pdus[i].tpdu_length, sca, dst, PDU_DCS_ALPHABET_UCS2, msg + off, n, n * 2, valid_minutes, srr, &udh);
            if (curlen < 0) {
                /* pdu_build sets chan_quectel_err */
                return -1;
            }
...
ssize_t pdu_build(uint8_t* buffer, size_t length, size_t* tpdulen, const char* sca, const char* dst, int dcs, const uint16_t* msg, unsigned msg_reallen,
                  unsigned msg_len, unsigned valid_minutes, int srr, const pdu_udh_t* udh)
{
    int len = 0;
    ...
    int dst_toa;
    int pdutype =
        PDUTYPE_MTI_SMS_SUBMIT | PDUTYPE_RD_ACCEPT | PDUTYPE_VPF_RELATIVE | PDUTYPE_SRR_NOT_REQUESTED | PDUTYPE_UDHI_NO_HEADER | PDUTYPE_RP_IS_NOT_SET;
    int use_udh = udh->parts > 1;
    int res;
    if (use_udh) {
        pdutype |= PDUTYPE_UDHI_HAS_HEADER;
    }
    ...
    if (dcs == PDU_DCS_ALPHABET_UCS2) {
        ...
        len += (gsm7_pack(msg, msg_reallen, (char*)(buffer + len), length - len - 1, msg_padding) + 1) / 2;
    }
...
int tpdu_parse_deliver(uint8_t* pdu, size_t pdu_length, int tpdu_type, char* oa, size_t oa_len, char* scts, uint16_t* msg, pdu_udh_t* udh)
{
    ...
    if (alphabet == PDU_DCS_ALPHABET_7BIT) {
        out_len = gsm7_unpack_decode((const char*)(pdu + i), udl_nibbles, msg, 1024 /* assume enough memory, as SMS messages are limited in size */,
