...
static int ports_match(const struct pdiscovery_ports* p1, const struct pdiscovery_ports* p2)
{
	...
	for (i = 0; i < ITEMS_OF(p1->ports); i++) {
		if (!p1->ports[i] || !p2->ports[i] || strcmp(p1->ports[i], p2->ports[i]) != 0) { return 0; }
	}
	return i;
}
...
{
	if (src->imei) { dst->imei = ast_strdup(src->imei); }
	if (src->imsi) { dst->imsi = ast_strdup(src->imsi); }
}
...
static int cache_lookup(struct discovery_cache* cache, const struct pdiscovery_request* req,
...
static char* pdiscovery_find_port(const char* name, int len, const char* subdir, unsigned* interface)
{
	...
	if (stat(name2, &statb) == 0 && S_ISDIR(statb.st_mode)) { port = pdiscovery_interface(name2, len2, interface); }
	return port;
}

#/* */

static int pdiscovery_interfaces(const char* devname, const char* name, int len, const struct pdiscovery_device* device,
					...
					for (idx = 0; idx < (int)ITEMS_OF(device->interfaces); idx++) {
						if (device->interfaces[idx] == interface) {
							if (ports->ports[idx] == NULL) {
								...
								if (++found == INTERFACE_TYPE_NUMBERS) { break; }
							} else {
								ast_debug(4,
										  "[%s discovery] port %s for bInterfaceNumber %02x already exists new is %s\n",
...
static const struct pdiscovery_device* pdiscovery_lookup_ids(const char* devname, const char* name, int len)
{
	...
	if (pdiscovery_get_id(name, len, "idVendor", &vid) == 1 && pdiscovery_get_id(name, len, "idProduct", &pid) == 1) {
		...
		for (idx = 0; idx < ITEMS_OF(device_ids); idx++) {
			if (device_ids[idx].vendor_id == vid && device_ids[idx].product_id == pid) { return &device_ids[idx]; }
		}
	}
	return NULL;
}
...
static char* pdiscovery_handle_ati(const char* str)
{
	...
	if (sscanf(str, "AT+GSN %15c OK", imei) == 1) { return ast_strdup(imei); }

	if (sscanf(str, " %15c OK", imei) == 1) { return ast_strdup(imei); }
...
static char* pdiscovery_handle_cimi(const char* str)
{
	...
	if (sscanf(str, "AT+CIMI %15c OK", imsi) == 1) { return ast_strdup(imsi); }

	if (sscanf(str, " %15c OK", imsi) == 1) { return ast_strdup(imsi); }
...
static int pdiscovery_handle_response(const struct pdiscovery_request* req, const struct iovec* iov, int iovcnt,
									  struct pdiscovery_result* res)
{
	int done = 0;
	char* str;
	...
	if (len > 0) {
		...
		if (done && req->imei && res->imei == NULL) { res->imei = pdiscovery_handle_ati(str); }
		if (done && req->imsi && res->imsi == NULL) { res->imsi = pdiscovery_handle_cimi(str); }
...
static int pdiscovery_do_cmd(const struct pdiscovery_request* req, int fd, const char* name, const char* cmd,
							 unsigned length, struct pdiscovery_result* res)
{
...
static int pdiscovery_get_info_cached(const char* port, const struct pdiscovery_request* req,
									  struct pdiscovery_result* res)
{
	int fail = 1;
...
static int pdiscovery_check_req(const struct pdiscovery_request* req, struct pdiscovery_result* res)
{
	...
	if (pdiscovery_read_info(req, res) == 0) {
		...
		ast_debug(4, "[%s discovery] %smatched IMEI=%s/%s IMSI=%s/%s\n", req->name, match ? "" : "un",
				  S_OR(req->imei, ""), S_OR(res->imei, ""), S_OR(req->imsi, ""), S_OR(res->imsi, ""));
	}
...
static int pdiscovery_check_device(const char* name, int len, const char* subdir, const struct pdiscovery_request* req,
								   struct pdiscovery_result* res)
{
	int len2;
	char* name2;
	...
	if (!found) { result_free(res); }
	return found;
}

#/* */

static int pdiscovery_request_do(const char* name, int len, const struct pdiscovery_request* req,
								 struct pdiscovery_result* res)
{
	int found = 0;
	struct dirent* dentry;
