...
at_enqueue_generic(struct cpvt* cpvt, at_cmd_t cmd, int prio, const char* format, ...)
{
	va_list ap;
	int rv;
	...
	if (!rv) { rv = at_queue_insert(cpvt, &at_cmd, 1, prio); }
	return rv;
}
...
int at_enqueue_initialization(struct cpvt* cpvt)
{
	...
	static const at_queue_cmd_t st_cmds[] = {
		...
		ATQ_CMD_DECLARE_ST(
			CMD_AT_CPIN,
			cmd_cpin), /* Check is password authentication requirement and the remainder validation times */
...
int at_enqueue_initialization_quectel(struct cpvt* cpvt, unsigned int dsci)
{
	...
	const at_queue_cmd_t cmds[] = {
		...
		ATQ_CMD_DECLARE_ST(
			CMD_AT_CVOICE,
			cmd_qpcmv), /* read the current voice mode, and return sampling rate、data bit、frame period */
...
int at_enqueue_cspn_cops(struct cpvt* cpvt)
{
	...
	static at_queue_cmd_t at_cmds[] = {ATQ_CMD_DECLARE_STI(CMD_AT_CSPN, cmd_cspn),
 ...
 */
int at_enqueue_sms(struct cpvt* cpvt, const char* destination, const char* msg, unsigned validity_minutes,
				   int report_req, const char* payload, size_t payload_len)
{
	struct pvt* const pvt = cpvt->pvt;

	/* set default validity period */
	if (validity_minutes <= 0) { validity_minutes = 3 * 24 * 60; }
	...
	pdu_part_t pdus[255];
	res = pdu_build_mult(pdus, "" /* pvt->sms_scenter */, destination, msg_ucs2, res, validity_minutes, !!report_req,
						 csmsref);
	if (res < 0) {
	...
	const int uid =
		smsdb_outgoing_add(pvt->imsi, destination, res, validity_minutes * 60, report_req, payload, payload_len);
	if (uid < 0) {
		chan_quectel_err = E_SMSDB;
		return -1;
	}
...
int at_enqueue_dial(struct cpvt* cpvt, const char* number, int clir)
{
	...
	if (clir != -1) {
		const int err = at_fill_generic_cmd(&cmds[cmdsno], "AT+CLIR=%d\r", clir);
		if (err) {
			chan_quectel_err = E_UNKNOWN;
			return -1;
		}
	...
	const int err = at_fill_generic_cmd(&cmds[cmdsno], "ATD%s;\r", number);
	...
	switch (cpvt->state) {
		...
		default:
			ast_log(LOG_ERROR, "[%s] Request answer for call idx %d with state '%s'\n", PVT_ID(cpvt->pvt),
					cpvt->call_idx, call_state2str(cpvt->state));
			chan_quectel_err = E_UNKNOWN;
			return -1;
	}
...
{
	if (cpvt->state == CALL_STATE_ACTIVE) { return 0; }
	...
	if (at_fill_generic_cmd(&cmd, "AT+CHLD=2%d\r", cpvt->call_idx) != 0) {
		chan_quectel_err = E_UNKNOWN;
		return -1;
	}
...
int at_enqueue_ping(struct cpvt* cpvt)
{
	...
	if (CONF_SHARED(pvt, query_time)) { ping_cmd = pvt->is_simcom ? PING_SIMCOM : PING_QUECTEL; }
...
void at_sms_retrieved(struct cpvt* cpvt, int confirm)
{
	...
	if (pvt->incoming_sms_index >= 0) {
		if (CONF_SHARED(pvt, autodeletesms)) { at_enqueue_delete_sms(cpvt, pvt->incoming_sms_index, TRIBOOL_NONE); }
		if (confirm) {
...
int at_enqueue_msg_direct(struct cpvt* cpvt, int directflag)
{
	...
	if (at_queue_insert_const(cpvt, directflag ? &cmd_direct : &cmd_indirect, 1, 0) != 0) {
		chan_quectel_err = E_QUEUE;
		return -1;
	}
	return 0;
...
static int at_enqueue_chup(struct cpvt* const cpvt)
{
	...
	static const at_queue_cmd_t at_chup =
		ATQ_CMD_DECLARE_STFT(CMD_AT_CHUP, RES_OK, cmd_chup, ATQ_CMD_FLAG_DEFAULT, ATQ_CMD_TIMEOUT_LONG, 0);
...
int at_enqueue_hangup(struct cpvt* cpvt, int call_idx, int release_cause)
{
	...
	if (cpvt == &pvt->sys_chan || cpvt->dir == CALL_DIR_INCOMING ||
		...
		if (PVT_STATE(pvt, chansno) > 1) {
			...
			static const at_queue_cmd_t cmd =
				ATQ_CMD_DECLARE_STFT(CMD_AT_CHLD_1x, RES_OK, cmd_chld1x, ATQ_CMD_FLAG_DEFAULT, ATQ_CMD_TIMEOUT_LONG, 0);
...
int at_enqueue_query_qgains(struct cpvt* cpvt)
{
	...
	if (at_queue_insert_const_at_once(cpvt, cmds, ITEMS_OF(cmds), 0) != 0) {
		chan_quectel_err = E_QUEUE;
		return -1;
	}
...
int at_enqueue_cgains(struct cpvt* cpvt, int txgain, int rxgain)
{
	...
	if (txgain >= 0) {
		const int err = at_fill_generic_cmd(&cmds[0], "+COUTGAIN=%d", txgain);
		if (err) {
			chan_quectel_err = E_UNKNOWN;
			return err;
		}
	...
	if (rxgain >= 0) {
		const int err = at_fill_generic_cmd(&cmds[1], "+CMICGAIN=%d", rxgain);
		if (err) {
			chan_quectel_err = E_UNKNOWN;
			return err;
		}
...
int at_enqueue_cmicgain(struct cpvt* cpvt, int gain)
{
	...
	const int err = at_fill_generic_cmd(&cmd, "AT+CMICGAIN=%d\r", gain);
	if (err) {
		chan_quectel_err = E_UNKNOWN;
		return err;
	}
...
int at_enqueue_coutgain(struct cpvt* cpvt, int gain)
{
	...
	const int err = at_fill_generic_cmd(&cmd, "AT+COUTGAIN=%d\r", gain);
	if (err) {
		chan_quectel_err = E_UNKNOWN;
		return err;
	}
...
int at_enqueue_cpcmreg(struct cpvt* cpvt, int reg)
{
	...
	if (at_queue_insert_const(cpvt, reg ? &cmd1 : &cmd0, 1, 1) != 0) {
		chan_quectel_err = E_QUEUE;
		return -1;
	}
...
int at_enqueue_cpcmfrm(struct cpvt* cpvt, int frm)
{
	...
	if (at_queue_insert_const(cpvt, frm ? &cmd1 : &cmd0, 1, 0) != 0) {
		chan_quectel_err = E_QUEUE;
		return -1;
	}
