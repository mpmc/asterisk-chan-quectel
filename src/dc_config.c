...
static unsigned int parse_on_off(const char* const name, const char* const value, unsigned int defval)
{
	...
	if (ast_true(value)) { return 1u; }
	if (ast_false(value)) { return 0u; }
...
void dc_sconfig_fill(struct ast_config* cfg, const char* cat, struct dc_sconfig* config)
{
	...
	for (v = ast_variable_browse(cfg, cat); v; v = v->next) {
		if (!strcasecmp(v->name, "context")) {
			...
			if (str2gain(v->value, &config->rxgain)) { config->rxgain = -1; }
		} else if (!strcasecmp(v->name, "txgain")) {
			if (str2gain(v->value, &config->txgain)) { config->txgain = -1; }
			...
			if (config->callingpres == -1) {
				...
				if (config->callingpres == 0 && errno == EINVAL) { config->callingpres = -1; }
			}
			...
			if (val == DEV_STATE_STOPPED || val == DEV_STATE_STARTED || val == DEV_STATE_REMOVED) {
				...
				ast_log(
					LOG_ERROR,
					"Invalid value for 'initstate': '%s', must be one of 'stop' 'start' 'remove' default is 'start'\n",
					v->value);
			}
		} else if (!strcasecmp(v->name, "callwaiting")) {
			if (strcasecmp(v->value, "auto")) { config->callwaiting = parse_on_off(v->name, v->value, 0u); }
			...
			if (config->dtmf_duration <= 0 && errno == EINVAL) { config->dtmf_duration = DEF_DTMF_DURATION; }
...
void dc_gconfig_fill(struct ast_config* cfg, const char* cat, struct dc_gconfig* config)
{
	...
	if (smsdb) { ast_copy_string(config->sms_db, smsdb, sizeof(config->sms_db)); }
