...
{
	for (; *number; number++) {
		if ((*number >= '0' && *number <= '9') || *number == '*' || *number == '#') { continue; }
		return 0;
	}
	return 1;
}
...
struct pvt* get_pvt(const char* dev_name, int online)
{
	...
	if (pvt) {
		if (pvt->connected && (!online || (pvt->initialized && pvt->gsm_registered))) { return pvt; }
		free_pvt(pvt);
	}
...
int send_sms(const char* const dev_name, const char* const number, const char* const message, int validity, int report,
			 const char* const payload, size_t payload_len)
{
...
int str2gain(const char* s, int* gain)
{
	...
	if (errno == ERANGE || g < 0 || g > MAX_GAIN) { return -1; }
	*gain = g;
	return 0;
}
...
int str2gain_simcom(const char* s, int* gain)
{
	...
	const int g = (int)strtol(s, NULL, 10);
	if (errno == ERANGE || g < 0 || g > MAX_GAIN_SIMCOM) { return -1; }
	*gain = g;
	return 0;
}
