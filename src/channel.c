...
int channels_loop(struct pvt* pvt, const struct ast_channel* requestor)
{
	...
	struct cpvt* tmp;
	return (requestor && ast_channel_tech(requestor) == &channel_tech && (tmp = ast_channel_tech_pvt(requestor)) &&
			tmp->pvt == pvt) ?
			   1 :
			   0;
}
...
static struct ast_channel* channel_request(attribute_unused const char* type, struct ast_format_cap* cap,
...
static struct ast_channel* channel_request(attribute_unused const char* type, format_t format,
										   const struct ast_channel* requestor, void* data, int* cause)
#else                                /* 1.8- */
...
/* TODO: add check when request 'holdother' what requestor is not on same device for 1.6 */
	...
	*cause = parse_dial_string(dest_dev, &dest_num, &opts);
	...
	if (pvt) {
#if ASTERISK_VERSION_NUM >= 120000 /* 12+ */
		channel = new_channel(pvt, AST_STATE_DOWN, NULL, pvt_get_pseudo_call_idx(pvt), CALL_DIR_OUTGOING,
							  CALL_STATE_INIT, NULL, assignedids, requestor, local_channel);
#else  /* 12- */
		channel = new_channel(pvt, AST_STATE_DOWN, NULL, pvt_get_pseudo_call_idx(pvt), CALL_DIR_OUTGOING,
							  CALL_STATE_INIT, NULL, requestor, local_channel);
#endif /* ^12- */
		ast_mutex_unlock(&pvt->lock);
		if (!channel) {
		...
		ast_log(LOG_WARNING, "[%s] Request to call on device %s\n", dest_dev,
				exists ? "which can not make call at this moment" : "not exists");
		*cause = AST_CAUSE_REQUESTED_CHAN_UNAVAIL;
	}
...
#endif /* ^11- */
{
	...
	if (parse_dial_string(dest_dev, &dest_num, &opts)) { return -1; }
...
static void activate_call(struct cpvt* cpvt)
{
	...
	if (CPVT_TEST_FLAG(cpvt, CALL_FLAG_MASTER)) { return; }
	...
	if (!CPVT_TEST_FLAG(cpvt, CALL_FLAG_ACTIVATED)) {
		...
		if (CONF_SHARED(pvt, multiparty)) { mixb_attach(&pvt->a_write_mixb, &cpvt->mixstream); }
	}
...
static int channel_answer(struct ast_channel* channel)
{
	...
	if (cpvt->dir == CALL_DIR_INCOMING) {
		if (at_enqueue_answer(cpvt)) { ast_log(LOG_ERROR, "[%s] Error sending answer commands\n", PVT_ID(pvt)); }
	}
...
static int channel_digit_end(attribute_unused struct ast_channel* channel, attribute_unused char digit,
							 attribute_unused unsigned int duration)
{
	return 0;
}
...
static inline void change_audio_endianness_to_le(attribute_unused struct iovec* iov, attribute_unused int iovcnt)
{
	...
	for (; iovcnt-- > 0; ++iov) { ast_swapcopy_samples(iov->iov_base, iov->iov_base, iov->iov_len / 2); }
#endif
}
...
static void timing_write_tty(struct pvt* pvt, size_t frame_size)
{
	...
	if (msg) { ast_debug(7, msg, PVT_ID(pvt)); }

	//	}


	if (iov_write(pvt, pvt->audio_fd, iov, iovcnt) >= 0) { PVT_STAT(pvt, write_frames)++; }
...
static void write_conference(struct pvt* pvt, const char* const buffer, size_t length)
{
	...
	AST_LIST_TRAVERSE(&pvt->chans, cpvt, entry) {
		if (CPVT_IS_ACTIVE(cpvt) && !CPVT_IS_MASTER(cpvt) && CPVT_TEST_FLAG(cpvt, CALL_FLAG_MULTIPARTY) &&
			cpvt->rd_pipe[PIPE_WRITE] >= 0) {
...
static struct ast_frame* channel_read_tty(struct cpvt* cpvt, struct pvt* pvt, size_t frame_size,
										  const struct ast_format* const fmt)
{
...
static void show_alsa_state(struct pvt* pvt, snd_pcm_t* const pcm, const char* pcm_desc, int dlvl)
{
	...
	if (res < 0) {
		...
		if (res < 0) {
			ast_log(LOG_ERROR, "[%s][ALSA][%s] Unable to get buffer sizes: %s", PVT_ID(pvt), pcm_desc,
					snd_strerror(res));
		}
		...
		ast_debug(dlvl, "[%s][ALSA][%s] Status - state:%s delay:%ld avail:%lu avail_max:%lu sdelay:%d buf:%lu|%lu\n",
				  PVT_ID(pvt), pcm_desc, snd_pcm_state_name(pcm_state), delay, avail, avail_max, sdelay, period_size,
				  buffer_size);
	}
}

static struct ast_frame* channel_read_uac(struct cpvt* cpvt, struct pvt* pvt, size_t frames,
										  const struct ast_format* const fmt)
{
	int res;
		...
		case SND_PCM_STATE_SETUP:
			...
			if (res) {
				ast_log(LOG_ERROR, "[%s][ALSA][CAPTURE] Prepare failed - state:%s err:'%s'\n", PVT_ID(pvt),
						snd_pcm_state_name(state), snd_strerror(res));
				return NULL;
			}
			break;
...
static struct ast_frame* channel_read(struct ast_channel* channel)
{
	...
	if (!cpvt || cpvt->channel != channel || !cpvt->pvt || cpvt->local_channel) { return &ast_null_frame; }

	struct pvt* const pvt = cpvt->pvt;

	while (ast_mutex_trylock(&pvt->lock)) { CHANNEL_DEADLOCK_AVOIDANCE(channel); }
	...
	if (CONF_UNIQ(pvt, uac) == TRIBOOL_FALSE && (!CPVT_IS_SOUND_SOURCE(cpvt) || pvt->audio_fd < 0)) { goto m_unlock; }
							 ...
							 size_t frame_size)
{
	if (CPVT_TEST_FLAG(cpvt, CALL_FLAG_MULTIPARTY) && !CPVT_TEST_FLAG(cpvt, CALL_FLAG_BRIDGE_CHECK)) {
		...
		if (bridged && ast_channel_tech(bridged) == &channel_tech && (tmp_cpvt = ast_channel_tech_pvt(bridged)) &&
			tmp_cpvt->pvt == pvt) {
			CPVT_SET_FLAGS(cpvt, CALL_FLAG_BRIDGE_LOOP);
			CPVT_SET_FLAGS((struct cpvt*)ast_channel_tech_pvt(bridged), CALL_FLAG_BRIDGE_LOOP);
			ast_log(LOG_WARNING,
					"[%s] Bridged channels %s and %s working on same device, discard writes to avoid voice loop\n",
	...
	if (CONF_SHARED(pvt, multiparty)) {  // use mix buffer
		...
		*/
		...
		if (iov_write(pvt, pvt->audio_fd, &iov, 1) >= 0) { PVT_STAT(pvt, write_frames)++; }
	}
...
static int channel_write(struct ast_channel* channel, struct ast_frame* f)
{
	...
	if (!cpvt || cpvt->channel != channel || !cpvt->pvt || cpvt->local_channel) { return 0; }
	...
	if (CPVT_TEST_FLAG(cpvt, CALL_FLAG_BRIDGE_LOOP)) { return 0; }
...
static int channel_fixup(struct ast_channel* oldchannel, struct ast_channel* newchannel)
{
	...
	ast_mutex_lock(&pvt->lock);
	if (cpvt->channel == oldchannel) { cpvt->channel = newchannel; }
	ast_mutex_unlock(&pvt->lock);
...
static int channel_indicate(struct ast_channel* channel, int condition, const void* data,
							attribute_unused size_t datalen)
{
...
void change_channel_state(struct cpvt* cpvt, unsigned newstate, int cause)
{
	...
	if (CONF_SHARED(pvt, multiparty)) {
		ast_debug(1, "[%s] Call - idx:%d channel:%s mpty:%d [%s] \xE2\x86\x92 [%s]\n", PVT_ID(pvt), call_idx,
				  channel ? "attached" : "detached", CPVT_TEST_FLAG(cpvt, CALL_FLAG_MULTIPARTY) ? 1 : 0,
				  call_state2str(oldstate), call_state2str(newstate));
	} else {
		ast_debug(1, "[%s] Call - idx:%d channel:%s [%s] \xE2\x86\x92 [%s]\n", PVT_ID(pvt), call_idx,
				  channel ? "attached" : "detached", call_state2str(oldstate), call_state2str(newstate));
	}
	...
	if (!channel) {
		...
		if (newstate == CALL_STATE_RELEASED) { cpvt_free(cpvt); }
	} else {
		/* for live channel */
		switch (newstate) {
			case CALL_STATE_DIALING:
...
#if ASTERISK_VERSION_NUM >= 120000 /* 12+ */
struct ast_channel* new_channel(struct pvt* pvt, int ast_state, const char* cid_num, int call_idx, unsigned dir,
								call_state_t state, const char* dnid, const struct ast_assigned_ids* assignedids,
								attribute_unused const struct ast_channel* requestor, unsigned local_channel)
#else  /* 13- */
struct ast_channel* new_channel(struct pvt* pvt, int ast_state, const char* cid_num, int call_idx, unsigned dir,
								call_state_t state, const char* dnid,
								attribute_unused const struct ast_channel* requestor, unsigned local_channel)
#endif /* ^13- */
{
	...
	if (cpvt) {
#if ASTERISK_VERSION_NUM >= 120000 /* 12+ */
		channel = ast_channel_alloc(1, ast_state, cid_num, PVT_ID(pvt), NULL, dnid, CONF_SHARED(pvt, context),
									assignedids, requestor, 0, "%s/%s-%02u%08lx", channel_tech.type, PVT_ID(pvt),
									call_idx, pvt->channel_instance);
#elif ASTERISK_VERSION_NUM >= 10800 /* 1.8+ */
		channel = ast_channel_alloc(1, ast_state, cid_num, PVT_ID(pvt), NULL, dnid, CONF_SHARED(pvt, context),
									requestor ? ast_channel_linkedid(requestor) : NULL, 0, "%s/%s-%02u%08lx",
									channel_tech.type, PVT_ID(pvt), call_idx, pvt->channel_instance);
#else                               /* 1.8- */
		channel = ast_channel_alloc(1, ast_state, cid_num, PVT_ID(pvt), NULL, dnid, CONF_SHARED(pvt, context), 0,
		...
		if (channel) {
			...
			if (!ast_strlen_zero(dnid)) { setvar_helper(pvt, channel, "CALLERID(dnid)", dnid); }

			ast_jb_configure(channel, &CONF_GLOBAL(jbconf));

			if (state != CALL_STATE_INIT) { change_channel_state(cpvt, state, AST_CAUSE_NORMAL_UNSPECIFIED); }
...
int queue_hangup(struct ast_channel* channel, int hangupcause)
{
	int rv = 0;
	if (channel) {
		if (hangupcause != 0) { ast_channel_hangupcause_set(channel, hangupcause); }
...
void start_local_report_channel(struct pvt* pvt, const char* number, const struct ast_str* const payload,
								const char* ts, const char* dt, int success, const char report_type,
...
void start_local_channel(struct pvt* pvt, const char* exten, const char* number, const channel_var_t* vars)
{
...
#if ASTERISK_VERSION_NUM >= 120000 /* 12+ */
	channel = ast_request("Local", pvt->local_format_cap ? pvt->local_format_cap : channel_tech.capabilities, NULL,
						  NULL, channel_name, &cause);
	...
	if (channel) {
		...
		for (; !ast_strlen_zero(vars->name); ++vars) { setvar_helper(pvt, channel, vars->name, vars->value); }
...
static int channel_func_read(struct ast_channel* channel, attribute_unused const char* function, char* data, char* buf,
							 size_t len)
{
	struct cpvt* cpvt = ast_channel_tech_pvt(channel);
	struct pvt* pvt;
...
static int channel_func_write(struct ast_channel* channel, const char* function, char* data, const char* value)
{
	...
	if (!strcasecmp(data, "callstate")) {
		...
		while (ast_mutex_trylock(&cpvt->pvt->lock)) { CHANNEL_DEADLOCK_AVOIDANCE(channel); }
		oldstate = cpvt->state;
