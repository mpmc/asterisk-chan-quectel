...
{
	if (!str) { return DEV_STATE_STOPPED; }
...
static int pcm_init(struct pvt* pvt, const char* dev, snd_pcm_stream_t stream, unsigned int rate, snd_pcm_t** pcm,
					unsigned int* pcm_channels)
{
	...
	snd_pcm_uframes_t period_size =
		adjust_uframes(((stream == SND_PCM_STREAM_CAPTURE) ? FRAME_SIZE_CAPTURE : FRAME_SIZE_PLAYBACK), rate);
...
static int alsa_status(snd_pcm_t* const pcm1, snd_pcm_t* const pcm2)
{
	...
	if (state == SND_PCM_STATE_DISCONNECTED) { return -1; }
...
static int lock_build(const char* devname, char* buf, unsigned length)
{
	...
	/* follow symlinks */
	if (realpath(devname, resolved_path) != NULL) { devname = resolved_path; }
		...
		if (len > 0) {
			...
			if (assigned > 0 && kill(len, 0) == 0) {
				if (len == getpid() && assigned > 1) {
					if (port_status(fd2, NULL) == 0) { pid = len; }
				} else {
					pid = len;
				}
			}
...
{
	if (ioctl(fd, TIOCNXCL) != 0) { ast_log(LOG_WARNING, "ioctl(TIOCNXCL) failed for %s: %s\n", dev, strerror(errno)); }
...
int opentty(const char* dev, char** lockfile, int typ)
{
	...
	if (ioctl(fd, TIOCEXCL) != 0) { ast_log(LOG_WARNING, "ioctl(TIOCEXCL) failed for %s: %s\n", dev, strerror(errno)); }
...
static int reopen_audio_port(struct pvt* pvt)
{
	...
	if (!PVT_NO_CHANS(pvt)) {
		struct cpvt* cpvt;
		AST_LIST_TRAVERSE(&(pvt->chans), cpvt, entry) { ast_channel_set_fd(cpvt->channel, 0, pvt->audio_fd); }
	}
...
static void disconnect_quectel(struct pvt* pvt)
{
	...
	if (pvt->initialized) {
		if (!pvt->is_simcom && CONF_UNIQ(pvt, uac) == TRIBOOL_TRUE) { at_disable_uac_immediately(pvt); }

		if (pvt->is_simcom && CONF_UNIQ(pvt, uac) == TRIBOOL_TRUE && pvt->has_voice) { at_cpcmreg_immediately(pvt, 0); }
	...
	if (CONF_UNIQ(pvt, data_tty)[0] == 0 && CONF_UNIQ(pvt, audio_tty)[0] == 0) {
		...
		if (resolved) {
			...
			ast_verb(3, "[%s]%s%s%s%s found on data_tty=%s audio_tty=%s\n", PVT_ID(pvt), imei[0] == 0 ? "" : " IMEI ",
					 imei, imsi[0] == 0 ? "" : " IMSI ", imsi, PVT_STATE(pvt, data_tty), PVT_STATE(pvt, audio_tty));
		} else {
...
static void pvt_start(struct pvt* pvt)
{
	...
	if (CONF_UNIQ(pvt, uac) > TRIBOOL_FALSE) {
		...
		pvt->audio_fd = opentty(PVT_STATE(pvt, audio_tty), pvt->is_simcom);
#endif
		if (pvt->audio_fd < 0) { goto cleanup_datafd; }
	}
...
static void* do_discovery(void* arg)
{
	...
	while (state->unloading_flag == 0) {
		...
		if (state->unloading_flag == 0) { sleep(SCONF_GLOBAL(state, discovery_interval)); }
	}
...
{
	if (state->discovery_thread == AST_PTHREADT_STOP) { return 0; }
...
static void discovery_stop(public_state_t* state)
{
	...
	ast_mutex_lock(&state->discovery_lock);
	if (state->discovery_thread && (state->discovery_thread != AST_PTHREADT_STOP) &&
...
int pvt_get_pseudo_call_idx(const struct pvt* pvt)
{
	...
	for (dwords = 1; dwords <= MAX_CALL_IDX; dwords++) {
		if (!TEST_BIT(bits, dwords)) { return dwords; }
	}
	return 0;
}
...
static int is_dial_possible2(const struct pvt* pvt, int opts, const struct cpvt* ignore_cpvt)
{
	...
	if (pvt->ring || pvt->cwaiting || pvt->dialing) { return 0; }

	AST_LIST_TRAVERSE(&pvt->chans, cpvt, entry) {
		switch (cpvt->state) {
			case CALL_STATE_INIT:
				if (cpvt != ignore_cpvt) { return 0; }
				break;
			...
			case CALL_STATE_ACTIVE:
				if (hold || !use_call_waiting) { return 0; }
				active++;
				break;

			case CALL_STATE_ONHOLD:
				if (active || !use_call_waiting) { return 0; }
				hold++;
				break;
...
int pvt_enabled(const struct pvt* pvt)
{
	return pvt->current_state == DEV_STATE_STARTED &&
		   (pvt->desired_state == pvt->current_state || pvt->restart_time == RESTATE_TIME_CONVENIENT);
}
...
static int can_dial(struct pvt* pvt, int opts, const struct ast_channel* requestor)
{
	...
	if ((opts & CALL_FLAG_HOLD_OTHER) == CALL_FLAG_HOLD_OTHER && channels_loop(pvt, requestor)) { return 0; }
...
struct pvt* find_device_ex(struct public_state* state, const char* name)
{
	...
	AST_RWLIST_TRAVERSE(&state->devices, pvt, entry) {
		ast_mutex_lock(&pvt->lock);
		if (!strcmp(PVT_ID(pvt), name)) { break; }
		ast_mutex_unlock(&pvt->lock);
	}
	AST_RWLIST_UNLOCK(&state->devices);
...
struct pvt* find_device_by_resource_ex(struct public_state* state, const char* resource, int opts,
	...
	if (!state) {
		if (pvt->ring || PVT_STATE(pvt, chan_count[CALL_STATE_INCOMING])) {
			...
			state = "Waiting";
				   ...
				   (PVT_STATE(pvt, chan_count[CALL_STATE_INIT]) + PVT_STATE(pvt, chan_count[CALL_STATE_DIALING]) +
...
struct ast_str* pvt_str_state_ex(const struct pvt* pvt)
{
	...
	if (state) {
		...
		if (pvt->ring || PVT_STATE(pvt, chan_count[CALL_STATE_INCOMING])) { ast_str_append(&buf, 0, "Ring "); }
		...
		if (pvt->cwaiting || PVT_STATE(pvt, chan_count[CALL_STATE_WAITING])) { ast_str_append(&buf, 0, "Waiting "); }
		...
		if (pvt->incoming_sms_index >= 0) { ast_str_append(&buf, 0, "Incoming SMS "); }

		if (pvt->outgoing_sms) { ast_str_append(&buf, 0, "Outgoing SMS"); }

		if (ast_str_strlen(buf) == 0) { ast_str_append(&buf, 0, "%s", "Free"); }
	}
...
static int pvt_time4restate(const struct pvt* pvt)
{
	if (pvt->desired_state != pvt->current_state) {
		if (pvt->restart_time == RESTATE_TIME_NOW ||
			(PVT_NO_CHANS(pvt) && !pvt->outgoing_sms && pvt->incoming_sms_index >= 0)) {
			return 1;
		}
	}
	return 0;
...
static int reload_config(public_state_t* state, int recofigure, restate_time_t when, unsigned* reload_immediality)
{
	...
	if ((cfg = ast_config_load(CONFIG_FILE, config_flags)) == NULL) { return -1; }
	...
	if (reload_immediality) { *reload_immediality = reload_now; }
	return 0;
}
...
static void devices_destroy(public_state_t* state)
{
	...
	AST_RWLIST_WRLOCK(&state->devices);
	while ((pvt = AST_RWLIST_REMOVE_HEAD(&state->devices, entry))) { pvt_destroy(pvt); }
	AST_RWLIST_UNLOCK(&state->devices);
}
...
static int load_module()
{
	...
	if (gpublic) {
		...
		if (rv != AST_MODULE_LOAD_SUCCESS) { ast_free(gpublic); }
	} else {
...
static int public_state_init(struct public_state* state)
{
	...
	if (reload_config(state, 0, RESTATE_TIME_NOW, NULL) == 0) {
		...
		if (discovery_restart(state) == 0) {
			...
			if (!(channel_tech.capabilities = ast_format_cap_alloc(0))) { return AST_MODULE_LOAD_FAILURE; }
#else
			if (!(channel_tech.capabilities = ast_format_cap_alloc())) { return AST_MODULE_LOAD_FAILURE; }
#endif
...
void pvt_reload(restate_time_t when)
{
	...
	if (dev_reload > 0) { discovery_restart(gpublic); }
}
